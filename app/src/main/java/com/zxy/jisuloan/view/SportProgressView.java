 ...
 * on 2019/8/13
 * android 渐变的半圆进度条
 * https://blog.csdn.net/desireyaoo/article/details/77884451
 */
public class SportProgressView extends View {
    ...
    private void init() {
        ...
        marginBottom = getTextHeight(mTextPaint) + ViewUtil.dp2px(8);
...
//        Log.e(TAG, "onMeasure--->mWidth = " + mWidth + ",mHeight = " + mHeight);
    ...
    private int sweepAngle = 220;//圆弧的弧度

    @Override
    protected void onDraw(Canvas canvas) {
        int startAngle = 160;//圆弧起点的角度
//        Log.e(TAG, "onDraw--->");
        int left = mProgressWidth / 2 + getPaddingLeft()+2;
        int right = left + 2 * mProgressR - 2;
        int top = mHeight - marginBottom - mProgressR + 3;
...
//        canvas.drawArc(rect, 180, mProgress, false, mPaint);
        canvas.drawArc(rect, startAngle, mProgress, false, mPaint);
//        drawText(canvas);//画底部初始的文字和结束的文字
...
//        String text = (int) (progress / 180.f * 100) + "%";
        String text = mCurProgress + "";
        Paint.FontMetrics fm = mTextPaint.getFontMetrics();
        float baseline = mHeight / 2 + marginBottom - fm.descent - ViewUtil.dp2px(40);//设置进度文字的位置
...
//        ValueAnimator animator = ValueAnimator.ofFloat(0, mCurProgress / 100f * 180);
        ValueAnimator animator = ValueAnimator.ofFloat(0, mCurProgress / (float) mMaxProgress * sweepAngle);
